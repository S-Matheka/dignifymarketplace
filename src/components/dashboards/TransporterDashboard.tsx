...
import { Truck, MapPin, DollarSign, Bell, MessageCircle } from 'lucide-react';
...
const TransporterDashboard: React.FC = () => {
  ...
  const [view, setView] = useState<'dashboard' | 'available' | 'history'>('dashboard');
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mt-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Transporter Actions</h2>
          <div className="flex flex-wrap gap-4">
            <button className="btn-primary" onClick={() => setView('available')}>Available Deliveries</button>
            <button className="btn-secondary" onClick={() => setView('history')}>Delivery History</button>
            {view !== 'dashboard' && (
              <button className="btn-secondary" onClick={() => setView('dashboard')}>Back to Dashboard</button>
            )}
          </div>
        </div>
        {view === 'available' && <AvailableDeliveries />}
